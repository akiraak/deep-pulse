...
3. **生成プラン作成** — 収集した情報を元に記事の構成案を `output/YYYY-MM-DD_タイトル_plan.md` として保存し、ユーザーに確認を取る。プランファイルには以下を含めること:
...
5. **静的サイトビルド** — 記事生成後、`npm run build:site` を実行して HTML を生成する。`dist_site/` に記事の HTML とトップページ（`index.html`）のリンクが正しく出力されていることを確認する
6. **コミット** — 記事の Markdown（`output/`）をコミットする（`dist_site/` は `.gitignore` に含まれるため、コミット対象外。GitHub Pages デプロイは CI が `npm run build:site` を実行して行う）
...
- **登場人物が多いテーマの場合** — 記事冒頭に「登場人物と相関図」セクションを設ける:
  ...
  - 関係性を**テキストベースの相関図**（コードブロック内）で可視化する
