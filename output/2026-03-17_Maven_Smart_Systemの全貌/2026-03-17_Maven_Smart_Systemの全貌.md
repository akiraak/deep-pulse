...
| **米国防総省（DoD）** | 政府機関 | MSSの発注者。「全合法用途への無制限アクセス」を要求 |
...
flowchart TD
    ...
    PAL -->|"MSSにClaude統合"| ANT
    PAL -->|"13億ドル契約"| DOD
    ANT -->|"自律兵器・監視に制限要求"| DOD
    DOD -->|"サプライチェーンリスク認定"| ANT
    ANT -->|"提訴"| WH
    MS -->|"差止め請求支持"| ANT
    DOD -->|"代替契約"| OAI
    DOD -->|"代替契約"| XAI
...
```chart
{
  "type": "bar",
  "data": {
    "labels": ["2017年（初期資金）", "2024年（初期契約）", "2025年（契約拡大）"],
    "datasets": [
      {
        "label": "契約額（億ドル）",
        "data": [0.7, 4.8, 12.75],
        "backgroundColor": ["#4a5568", "#2b6cb0", "#2c5282"]
      }
    ]
  },
  "options": {
    "plugins": {
      "title": { "display": true, "text": "Maven Smart System 契約額の推移（億ドル）" }
    },
    "scales": {
      "y": { "beginAtZero": true, "title": { "display": true, "text": "億ドル" } }
    }
  }
}
```
...
flowchart LR
    ...
    subgraph mss["Maven Smart System"]
        FUSION["データ融合\n150+ソース統合"]
        AI["AI/ML処理\nClaude + CV"]
        UI["統合UI\n黄=標的 / 青=友軍"]
    end
...
- [Palantir MAVEN: DoD Deploys AI That Closes Kill Chains in Minutes](https://www.abhs.in/blog/palantir-maven-smart-system-ai-kill-chain-dod-deployment-2026)
- [Project Maven - Wikipedia](https://en.wikipedia.org/wiki/Project_Maven)
    ...
    "datasets": [
      {
        "label": "累計攻撃標的数",
        "data": [1000, 2000, 5500],
        "backgroundColor": ["#c53030", "#e53e3e", "#fc8181"]
      }
    ]
  },
  "options": {
    "plugins": {
      "title": { "display": true, "text": "Operation Epic Fury 攻撃規模の推移" }
    },
    "scales": {
      "y": { "beginAtZero": true, "title": { "display": true, "text": "累計標的数" } }
    }
  }
}
```
...
- [Question for Hegseth: Did US Military Rely on AI Targeting for Bombing of Iranian School?](https://www.commondreams.org/news/artificial-intelligence-iran-war)
...
```chart
{
  "type": "line",
  "data": {
    "labels": ["従来（人力のみ）", "2020年（初回演習）", "2024年（運用中）", "2026年（Epic Fury）"],
    "datasets": [
      {
        "label": "ターゲティング処理時間（分）",
        "data": [1440, 720, 1, 0.5],
  ...
  "options": {
    "plugins": {
      "title": { "display": true, "text": "キルチェーン処理時間の圧縮（分）" }
    },
    "scales": {
      "y": { "type": "logarithmic", "title": { "display": true, "text": "処理時間（分・対数スケール）" } }
    }
  }
}
```
...
- [US military leans into AI for attack on Iran, but the tech doesn't lessen the need for human judgment in war](https://theconversation.com/us-military-leans-into-ai-for-attack-on-iran-but-the-tech-doesnt-lessen-the-need-for-human-judgment-in-war-277831)
- [Maven Smart System](https://www.missiledefenseadvocacy.org/maven-smart-system/)
...
```mermaid
flowchart TD
    A["2024年11月\nAnthropic-Palantir-AWS提携発表"] --> B["2026年1月3日\nマドゥロ拘束作戦でClaude使用"]
    B --> C["1月12日\nヘグセス国防長官がAnthropicを暗に批判"]
    C --> D["2月13日\nWSJ報道でClaude使用が公に"]
    D --> E["2月17日\nSemafor: Palantir幹部が国防総省に通報"]
    E --> F["2月23日\nxAI、機密システム利用契約を締結"]
    F --> G["2月27日\nヘグセスがサプライチェーンリスク認定を発表"]
    G --> H["2月28日\nOpenAI、国防総省と契約発表\nOperation Epic Fury開始"]
    H --> I["3月5日\n正式にサプライチェーンリスク認定"]
    I --> J["3月9日\nAnthropic、トランプ政権を提訴"]
    J --> K["3月10日\nMicrosoft + 元軍高官22人がAnthropicを支持"]
```

| 日付 | 出来事 |
|---|---|
| 2月27日 | [ヘグセスがX（旧Twitter）でサプライチェーンリスク認定を発表](https://www.cnbc.com/2026/03/05/anthropic-pentagon-ai-claude-iran.html) |
| 2月28日 | [トランプが全連邦機関にAnthropic技術の使用即時停止を指示](https://www.cnbc.com/2026/03/05/anthropic-pentagon-ai-claude-iran.html)。トランプ:「犬のようにクビにした」 |
...
- [Microsoft and retired military chiefs back Anthropic in court fight against Pentagon](https://federalnewsnetwork.com/artificial-intelligence/2026/03/microsoft-backs-anthropic-urging-a-judge-to-halt-pentagons-actions-against-ai-company/)
...
| Human-in-the-loop | 🔴 45秒の承認は形骸化リスク | 🟢 制度上は人間が最終決定 |
| AI標的精度 | 🔴 砂漠地形で30%以下に低下 | 🟢 処理速度は劇的に向上 |
| 民間AI企業の軍事利用ルール | 🔴 調停する枠組みが不在 | — |
| 複数AIベンダーの確保 | 🟡 移行に12か月以上 | 🟢 OpenAI・xAIと契約済み |
| 国際法との整合性 | 🔴 UN専門家が「深刻なリスク」警告 | 🟢 NATOで標準化推進中 |
