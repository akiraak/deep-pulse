...
```mermaid
flowchart TB
    NC["NVIDIA NemoClaw<br/>CLI / プラグイン / ブループリント"]
    OS["NVIDIA OpenShell<br/>ゲートウェイ / ポリシー / 推論ルーティング"]
    OC["OpenClaw<br/>サンドボックス内のエージェント"]
...
flowchart LR
    subgraph Host["ホスト側"]
        User["ユーザー / オペレータ<br/>CLI・ダッシュボード"]
        Control["NemoClaw control<br/>オンボーディング / ブループリント"]
        Gateway["OpenShell gateway<br/>サンドボックス / ポリシー / ルーティング"]
        State["State and artifacts<br/>認証情報・ログ・ポリシー"]
    end
    subgraph Sandbox["サンドボックス"]
        Agent["NemoClaw sandbox<br/>OpenClaw + プラグイン"]
    end
    subgraph External["外部"]
        Inference["Inference<br/>NVIDIA / OpenAI / Anthropic 等"]
        Integrations["Integrations<br/>Telegram / Slack / GitHub / MCP"]
    end
...
| 8 | **Model Router**（実験的） | `NVIDIA_API_KEY` | ホスト側ポート 4000 | リクエストごとにモデル振り分け |
