...
title: "Architecture — NVIDIA NemoClaw Developer Guide"
---

# Architecture[\#](#architecture "Link to this heading"){.headerlink}
...
## System Overview[\#](#system-overview "Link to this heading"){.headerlink}
...
## Deployment Topology[\#](#deployment-topology "Link to this heading"){.headerlink}
...
Layering from top to bottom:
  ...
  Layer                Runs as                                                                         Role
  -------------------- ------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  Host CLI             Host process ([`nemoclaw`{.docutils .literal .notranslate}]{.pre} on Node.js)   Orchestrates OpenShell via [`openshell`{.docutils .literal .notranslate}]{.pre} CLI calls.
  ...
  OpenShell L7 proxy   Process in the gateway container                                                Intercepts agent egress and rewrites [`Authorization`{.docutils .literal .notranslate}]{.pre} headers (Bearer/Bot) and URL-path segments to inject the real credential at the network boundary.

NemoClaw never gives the sandbox a raw provider key. At onboard time it registers credentials with OpenShell's provider/placeholder system, and the L7 proxy substitutes the real value into outbound requests at egress. The CLI helper [`isInferenceRouteReady`{.docutils .literal .notranslate}]{.pre} (in [`src/lib/onboard.ts`{.docutils .literal .notranslate}]{.pre}) is a host-side readiness check used by the resume flow to decide whether the active route already covers the chosen provider and model --- it is not a runtime component.

For the DGX Spark-specific variant of this topology (cgroup v2, aarch64, unified memory), refer to the [NVIDIA Spark playbook](https://build.nvidia.com/spark/nemoclaw){.reference .external rel="noreferer noopener" target="_blank"}.

## NemoClaw Plugin[\#](#nemoclaw-plugin "Link to this heading"){.headerlink}

The plugin is a thin TypeScript package that registers an inference provider and the [`/nemoclaw`{.docutils .literal .notranslate}]{.pre} slash command. It runs in-process with the OpenClaw gateway inside the sandbox. It also registers runtime hooks that keep the agent aware of its environment. Before an agent turn starts, the plugin prepends a short context block with the active sandbox name, sandbox phase, network policy summary, and filesystem policy summary. When the policy or phase changes during a session, the plugin sends a smaller update block instead of repeating the full context.
...
## NemoClaw Blueprint[\#](#nemoclaw-blueprint "Link to this heading"){.headerlink}
...
### Blueprint Lifecycle[\#](#blueprint-lifecycle "Link to this heading"){.headerlink}
...
1.  Resolve. The plugin locates the blueprint artifact and checks the version against [`min_openshell_version`{.docutils .literal .notranslate}]{.pre} and [`min_openclaw_version`{.docutils .literal .notranslate}]{.pre} constraints in [`blueprint.yaml`{.docutils .literal .notranslate}]{.pre}.
...
4.  Apply. The runner executes the plan by calling [`openshell`{.docutils .literal .notranslate}]{.pre} CLI commands.

5.  Status. The runner reports current state.

## Sandbox Environment[\#](#sandbox-environment "Link to this heading"){.headerlink}

The sandbox runs the [[`ghcr.io/nvidia/openshell-community/sandboxes/openclaw`{.docutils .literal .notranslate}]{.pre}](https://github.com/NVIDIA/OpenShell-Community){.reference .external rel="noreferer noopener" target="_blank"} container image. Inside the sandbox:
...
-   Network egress is restricted by the baseline policy in [`openclaw-sandbox.yaml`{.docutils .literal .notranslate}]{.pre}.

-   Filesystem access is confined to [`/sandbox`{.docutils .literal .notranslate}]{.pre} and [`/tmp`{.docutils .literal .notranslate}]{.pre} for read-write access, with system paths read-only.
...
Some model and provider combinations need agent-specific compatibility setup. NemoClaw keeps those declarations under [`nemoclaw-blueprint/model-specific-setup/<agent>/`{.docutils .literal .notranslate}]{.pre} so OpenClaw and Hermes fixes can be tested and reviewed independently.

Refer to [[Inference Options]{.std .std-doc}](../inference/inference-options.html){.reference .internal} for provider configuration details.

## Provider Credential Storage[\#](#provider-credential-storage "Link to this heading"){.headerlink}

Provider credentials live in the OpenShell gateway store, not on the host filesystem. NemoClaw never writes them to host disk; the OpenShell L7 proxy injects values at egress. See [[Credential Storage]{.std .std-doc}](../security/credential-storage.html){.reference .internal} for the inspection, rotation, and migration flow.

## Host-Side State and Config[\#](#host-side-state-and-config "Link to this heading"){.headerlink}

NemoClaw keeps non-secret operator-facing state on the host rather than inside the sandbox.

  Path                                                                    Purpose
  ----------------------------------------------------------------------- -----------------------------------------------------------------------------------------
  [`~/.nemoclaw/sandboxes.json`{.docutils .literal .notranslate}]{.pre}   Registered sandbox metadata, including the default sandbox selection.
  [`~/.openclaw/openclaw.json`{.docutils .literal .notranslate}]{.pre}    Host OpenClaw configuration that NemoClaw snapshots or restores during migration flows.

The following environment variables configure optional services and local access.

  Variable                                                                  Purpose
  ------------------------------------------------------------------------- ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  [`TELEGRAM_BOT_TOKEN`{.docutils .literal .notranslate}]{.pre}             Telegram bot token you provide before [`nemoclaw`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`onboard`{.docutils .literal .notranslate}]{.pre}. OpenShell stores it in a provider; the sandbox receives placeholders, not the raw secret.
  [`TELEGRAM_ALLOWED_IDS`{.docutils .literal .notranslate}]{.pre}           Comma-separated Telegram user or chat IDs for allowlists when onboarding applies channel restrictions.
  [`CHAT_UI_URL`{.docutils .literal .notranslate}]{.pre}                    URL for the optional chat UI endpoint.
  [`NEMOCLAW_DISABLE_DEVICE_AUTH`{.docutils .literal .notranslate}]{.pre}   Build-time-only toggle that disables gateway device pairing when set to [`1`{.docutils .literal .notranslate}]{.pre} before the sandbox image is created.

For normal setup and reconfiguration, prefer [`nemoclaw`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal .notranslate}[`onboard`{.docutils .literal .notranslate}]{.pre} over editing these files by hand. Do not treat [`NEMOCLAW_DISABLE_DEVICE_AUTH`{.docutils .literal .notranslate}]{.pre} as a runtime setting for an already-created sandbox.
